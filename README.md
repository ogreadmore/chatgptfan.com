...
The build performs no network calls. It generates a branded sharing image and article metadata for every published Daily Brief. Failed feed refreshes retain the last successful stories and expose the source status on the News page. Search runs locally in the visitor's browser; all other content works without JavaScript. Fonts are self-hosted. No visitor accounts are included. Formspree contact is connected and tested. GA4 runs only on production: EEA, UK, and Swiss visitors are asked to opt in; other identified countries use automatic analytics with a footer opt-out. Saved declines, Global Privacy Control, and Do Not Track are respected. Country lookup failures leave analytics and the automatic prompt off. The country-only lookup uses api.country.is with a five-minute session cache. Integration settings live in `content/site.json`; private account setup notes stay outside the public repository.
